...
def playGame(wordList):
    ...
    flagoffed = False
    HAND_SIZE = input('Choose the hand size to want to play with (press enter to play default hand size): ')
    if HAND_SIZE == "":
        HAND_SIZE = 7
    else:
        while True:
            try:
                HAND_SIZE = abs(int(HAND_SIZE))
                break
            except ValueError:
                HAND_SIZE = input('Enter an integer number: ')
